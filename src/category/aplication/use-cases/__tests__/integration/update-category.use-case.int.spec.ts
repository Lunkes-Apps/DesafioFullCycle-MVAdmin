...
describe("Update category use case test", () => {
  ...
  test("Should update the name of a category in data base", async () => {
    const category1 = Category.create({ name: "Category A" });
    await repository.insert(Category.create(category1));

    await useCase.execute({
      id: category1.category_id.id,
      name: "New Category C",
    });
    let newCategory = await repository.findById(category1.category_id);
